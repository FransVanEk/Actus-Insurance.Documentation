...
A parametric mortality model where the force of mortality increases exponentially with age: μ(x) = A + B × exp(x × ln C). Widely used in actuarial science. In this project, it provides the base mortality rates for the life insurance projection kernel.
...
The transfer of data from CPU memory (host) to GPU memory (device). This is a necessary step before GPU computation can begin. The speed of H2D transfer is a key factor in overall GPU performance.

### D2H (Device-to-Host)

The transfer of results from GPU memory (device) back to CPU memory (host) after computation completes.
...
A risk metric that estimates the maximum expected loss at a given confidence level over a given time period. VaR99 means the loss that would not be exceeded 99% of the time.
